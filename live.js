...
window.WotwLive = (() => {
  ...
  function displayState(p) {
    return ['Schläft','Wach','OK'].includes(p.state) ? '' : (p.state || '');
  }
  ...
  function party() {
    return `<div class="lp-party">${snapshot.live.party.map(p=>`<article class="lp-party-card" data-character="${p.id}"><a class="lp-party-name" href="#/sheet/${p.id}">${portrait(p.id)}<div><h2>${esc(p.name)}</h2><span>${esc(p.class)}</span></div><span class="lp-arrow" aria-hidden="true">↗</span></a>${meter('HP',p.hp)}${extraResources(p,true)}<div class="lp-card-foot"><span>${esc(displayState(p))}</span><a href="#/sheet/${p.id}">Charakterbogen →</a></div></article>`).join('')}</div>`;
  }
  function statusLine() {
